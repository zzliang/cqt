...
//			});
		...
		var _userId;
		var _userName = "CQT";	//用于即时通讯（ 当前登录用户）
			...
			$.ajax({
				...
				success: function(data){
				 $.each(data.list, function(i, u){
					 //登陆者资料
					 $("#user_info").html('<small>Welcome</small> '+u.userName+'');
					 _userName = u.userName;
					 _userId = u.userId;//用户ID
					 hf(u.skin);//皮肤
					 if(u.userName != 'admin'){
		...
		function editUserH(){
			 ...
			 diag.Drag=true;
			 diag.Title ="个人资料";
			 diag.URL = locat+'/user/goEditU.do?userId='+_userId+'&fx=head';
			 diag.Width = 225;
			 diag.Height = 389;
		...
		function changeUser(userId){
			//alert(userId);
			window.location.href=locat+"/main/mUserList?userId="+_userId;
		}
