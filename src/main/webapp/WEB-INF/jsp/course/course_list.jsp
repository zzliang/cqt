...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="course/listCourse.do" name="courseForm" id="courseForm" method="post" >
			<table>
				<tr>
					<td>
						...
						<c:forEach items="${courseList}" var="course" varStatus="vs">
							<tr>
								<td class='center' style="width: 30px;">
									<label><input type='checkbox' name='ids' value="${course.courseId }"/><span class="lbl"></span></label>
								</td>
								...
								<td style="width: 60px;">
									<div class='hidden-phone visible-desktop btn-group'>
										<a class='btn btn-mini btn-info' title="编辑" onclick="editCourse('${course.courseId }');"><i class='icon-edit'></i></a>
										<a class='btn btn-mini btn-danger' title="删除" onclick="delCourse('${course.courseId }','${course.courseGroup.courseGroupName }');"><i class='icon-trash'></i></a>
									</div>
								</td>
							</tr>
						</c:forEach>
		...
		//新增
		function addCourse(){
			 $(location).attr('href', '<%=basePath%>course/goAddCourse.do');
		}
		...
		function addCourseGroup(){
			...
			diag.Drag=true;
			diag.Title ="添加课程组";
			diag.URL = '<%=basePath%>course/goAddCourseGroup.do';
			diag.Width = 500;
			diag.Height = 300;
		...
		function viewCourse(courseId){
			 ...
			 diag.Drag=true;
			 diag.Title ="查看";
			 diag.URL = '<%=basePath%>course/goViewCourse.do?courseId='+courseId;
			 diag.Width = 225;
			 diag.Height = 415;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('cqtwindow').style.display == 'none'){
		...
		function editCourse(courseId){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>course/goEditCourse.do?courseId='+courseId;
			 diag.Width = 225;
			 diag.Height = 415;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('cqtwindow').style.display == 'none'){
		...
		function editCourseGroup(courseGroupId){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>course/goEditCourseGroup.do?courseGroupId='+courseGroupId;
			 diag.Width = 500;
			 diag.Height = 300;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('cqtwindow').style.display == 'none'){
		...
		function delCourse(courseId,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>course/deleteCourse.do?courseId="+courseId+"&tm="+new Date().getTime();
