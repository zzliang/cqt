...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="courseConfig/list.do" name="courseConfigForm" id="courseConfigForm" method="post" >
			<table>
				<tr>
					<td>
		...
		function editCourseConfig(courseConfigId){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>courseConfig/goEdit.do?courseConfigId='+courseConfigId;
			 diag.Width = 400;
			 diag.Height = 415;
		...
		function delCourseConfig(courseConfigId,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>courseConfig/delete.do?courseConfigId="+courseConfigId+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(ids==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>courseConfig/deleteAll.do?tm='+new Date().getTime(),
