...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="school/listSchool.do" method="post" name="schoolForm" id="schoolForm">
			<table>
				<tr>
					<td>
						...
						<c:forEach items="${schoolList}" var="school" varStatus="vs">
							<tr>
								<td class='center' style="width: 30px;">
									<label><input type='checkbox' name='ids' value="${school.schoolId }" id="${school.schoolCode }" alt="${school.schoolName }"/><span class="lbl"></span></label>
								</td>
		...
		cqtPost = function (url,args){
	        var cqtfm = $("<form method='post'></form>");
	        cqtfm.attr({"action":url});
	        for (arg in args){
		...
		function addSchool(){
			<%--  top.jzts();
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>school/goAddSchool.do';
			 diag.Width = 500;
			 diag.Height = 600;
			 ...
			 diag.show(); --%>
			 //window.location.href='<%=basePath%>school/goAddSchool.do';
			 $(location).attr('href', '<%=basePath%>school/goAddSchool.do');
			 //cqtPost('<%=basePath%>school/goAddSchool.do',{aa:'11',bb:'22'});
		}
		...
		function viewSchool(schoolId){
			 ...
			 diag.Drag=true;
			 diag.Title ="查看";
			 diag.URL = '<%=basePath%>school/goViewSchool.do?schoolId='+schoolId;
			 diag.Width = 500;
			 diag.Height = 500;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('cqtwindow').style.display == 'none'){
		...
		function editSchool(schoolId){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>school/goEditSchool.do?schoolId='+schoolId;
			 diag.Width = 500;
			 diag.Height = 500;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('cqtwindow').style.display == 'none'){
		...
		function delSchool(schoolId,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>school/deleteSchool.do?schoolId="+schoolId+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(ids==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>school/deleteAllSchool.do?tm='+new Date().getTime(),
						    	data: {schoolIds:ids},
								dataType:'json',
								cache: false,
