package com.cqt.controller.system.head;
...
import com.cqt.util.SmsUtil;
import com.cqt.util.Tools;
import com.cqt.util.Watermark;
import com.cqt.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController {
	...
	public Object getUname() {
		...
		List<User> pdList = new ArrayList<User>();
		try {
			pdList.add(SessionContext.getUser());
		} catch (Exception e) {
	...
	public void setSKIN(PrintWriter out){
		...
		try{
			pd = this.getPageData();
			String userId = SessionContext.getUser().getUserId();
			pd.put("userId", userId);
			userService.updateUserByPd(pd);
			//同时更新会话中的用户信息
			SessionContext.getUser().setSkin(pd.getString("skin"));
			if(SessionContext.isMuserList()){
				for(User u : SessionContext.getMUserList()){
					if(u.getUserId().equals(userId)){
						u.setSkin(pd.getString("skin"));
					}
				}
			}else{
				SessionContext.getUserRole().setSkin(pd.getString("skin"));
			}
